...
- **Description**:
  - Canvas 只负责绘制棋盘和棋子，不处理任何交互事件
  - 绘制 12×5 棋盘：石质底纹、公路暗金线、铁路亮金粗线+流光粒子、行营蓝微光、大本营暗红高台
  - 停靠点圆形凹槽
  - 棋子渲染：己方玉质底座+发光刻印（等级越高越亮）、敌方暗子统一背面、白板未分配灰色无光、军旗金色旗帜+暗红脉冲、地雷暗紫裂纹+锁链、炸弹橙红燃烧符文、工兵青蓝齿轮
  - 行营护盾光效
  - 交互由 React 在 Canvas 上层覆盖透明交互层处理（见 Task 16）
...
- **Description**:
  - 交互层：在 Canvas 上方覆盖一层透明 React 交互层，所有点击/拖动事件由此层捕获并映射到棋盘坐标，Canvas 仅负责绘制
